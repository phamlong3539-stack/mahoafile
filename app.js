...
(() => {
  ...
  const _CipherShield = (() => {
    ...
    function _incBlockedProbes(reason) {
      _blockedProbesCount++;
      try {
        localStorage.setItem('cvlt_blocked_probes', _blockedProbesCount.toString());
        localStorage.setItem('cvlt_last_sec_incident', JSON.stringify({
          reason: reason || 'UNAUTHORIZED_PROBE',
  ...
  // ============================================================================
  // END OF TITANIUM ANTI-REVERSE SUITE — Application Core Engine begins below
  // ============================================================================
...
=====================================================`;
  ...
  class IpaStudioEngine {
    ...
    static inspectMachO(bytes) {
      ...
      return { arch, isEncrypted, is64, isFat };
    }

    /**
     * Parse all Mach-O load commands (LC_LOAD_DYLIB, LC_RPATH, LC_ID_DYLIB, LC_LOAD_WEAK_DYLIB)
     */
    static parseMachODependencies(bytes) {
      if (!bytes || bytes.length < 32) return [];
      const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
      const isLE = true; // iOS ARM64 is Little Endian

      const magic = view.getUint32(0, isLE);
      ...
      for (let i = 0; i < ncmds && cmdOffset + 8 <= bytes.length; i++) {
        const cmd = view.getUint32(cmdOffset, isLE);
        const cmdsize = view.getUint32(cmdOffset + 4, isLE);
        if (cmdsize < 8 || cmdOffset + cmdsize > bytes.length) break;

        // LC_LOAD_DYLIB (0x0C), LC_ID_DYLIB (0x0D), LC_LOAD_WEAK_DYLIB (0x18 | 0x80000018), LC_RPATH (0x1C | 0x8000001C), LC_REEXPORT_DYLIB (0x1F | 0x8000001F)
        const isLoadDylib = cmd === 0x0C;
        const isIdDylib = cmd === 0x0D;
        const isWeakDylib = cmd === 0x18 || cmd === 0x80000018;
        const isRpath = cmd === 0x1C || cmd === 0x8000001C;
        const isReexport = cmd === 0x1F || cmd === 0x8000001F;

        if (isLoadDylib || isIdDylib || isWeakDylib || isRpath || isReexport) {
          const strOffset = view.getUint32(cmdOffset + 8, isLE);
          if (strOffset < cmdsize) {
            const rawBytes = bytes.subarray(cmdOffset + strOffset, cmdOffset + cmdsize);
            let nullIdx = rawBytes.indexOf(0);
            if (nullIdx === -1) nullIdx = rawBytes.length;
            const pathStr = new TextDecoder('utf-8').decode(rawBytes.subarray(0, nullIdx));

            let typeLabel = 'LC_LOAD_DYLIB';
            let badgeClass = 'custom';
            if (isIdDylib) { typeLabel = 'LC_ID_DYLIB'; badgeClass = 'custom'; }
            else if (isRpath) { typeLabel = 'LC_RPATH'; badgeClass = 'rpath'; }
            else if (isWeakDylib) { typeLabel = 'LC_LOAD_WEAK_DYLIB'; badgeClass = 'custom'; }
            else if (isReexport) { typeLabel = 'LC_REEXPORT_DYLIB'; badgeClass = 'custom'; }
            else if (pathStr.startsWith('/usr/lib') || pathStr.startsWith('/System/Library')) { badgeClass = 'system'; }
            else if (pathStr.startsWith('@rpath')) { badgeClass = 'rpath'; }

            dependencies.push({
              cmd,
              cmdsize,
              cmdOffset,
              strOffset: cmdOffset + strOffset,
              maxLen: cmdsize - strOffset,
              type: typeLabel,
              badgeClass,
              path: pathStr
            });
    ...
    /**
     * Safely modify a dependency string inside Mach-O load command in-place
     */
    static modifyMachODependency(bytes, oldPath, newPath) {
      if (!bytes || !oldPath || !newPath) return false;
      const deps = this.parseMachODependencies(bytes);
      const target = deps.find(d => d.path === oldPath || d.path.includes(oldPath));
      if (!target) return false;

      const newBytes = new TextEncoder().encode(newPath);
      if (newBytes.length + 1 > target.maxLen) {
        // Fits within limit? If not, trim or replace up to available padded capacity
        return false;
      }

      // Overwrite string and fill remainder with zeros
      for (let i = 0; i < target.maxLen; i++) {
        if (i < newBytes.length) {
          bytes[target.strOffset + i] = newBytes[i];
        } else {
          bytes[target.strOffset + i] = 0;
        }
      }
      return true;
    }
    ...
    /**
     * Pre-Flight Anti-Crash Diagnostic Engine
     * Tests all 6 critical points that cause iOS Sideload Crashes (SIGABRT / dyld exit)
     */
    static async runFullAntiCrashDiagnostic(loadedIpa, injectedDylibs = [], options = {}) {
      const checks = [];
      let passedCount = 0;
      ...
      if (!loadedIpa) {
        return {
          ...
          checks: [{
            id: 'no_ipa',
            name: 'Chưa Nạp Tệp IPA',
            passed: false,
            severity: 'critical',
            message: 'Vui lòng kéo thả tệp IPA vào để thực hiện chẩn đoán cấu trúc.',
            detail: 'Không có dữ liệu nhị phân để phân tích.',
            autoFixable: false
          }]
        };
      }

      const { zip, appDir, appInfo, plistStr } = loadedIpa;

      // 1. FAIRPLAY DRM ENCRYPTION CHECK (Leading Cause of Immediate SIGKILL Crash)
      if (appInfo.isEncryptedDrm) {
        checks.push({
          id: 'drm_encrypted',
          name: 'Kiểm Tra Apple FairPlay DRM (Mã Hóa App Store)',
          passed: false,
          severity: 'critical',
          message: '❌ CẢNH BÁO NGUY HIỂM: Binary dính Apple FairPlay DRM (cryptid = 1)!',
          detail: 'Ứng dụng này được tải trực tiếp từ App Store và CHƯA được giải mã (Dump/Decrypt). Khi cài đặt qua Sideload, iOS Kernel sẽ bắn tín hiệu SIGKILL (CodeSigning / dyld exit) và VĂNG 100% khi vừa mở app!',
          recommendation: 'Hãy dùng bản IPA đã được giải mã sẵn (tải từ Decrypted App Store / ArmConverter / TrollFools) để sideload.',
          autoFixable: false
        });
      } else {
        passedCount++;
        checks.push({
          id: 'drm_encrypted',
          name: 'Kiểm Tra Apple FairPlay DRM',
          passed: true,
          severity: 'pass',
          message: '🟢 ĐẠT CHUẨN: Binary đã được giải mã (Decrypted / No DRM)',
          detail: 'Không phát hiện cờ mã hóa FairPlay (cryptid = 0). Cho phép chạy sideload an toàn.',
          autoFixable: false
        });
      }

      // 2. CPU ARCHITECTURE & FAT BINARY ALIGNMENT CHECK
      const execEntry = zip.file(`${appDir}${appInfo.executableName}`);
      let execBytes = null;
      if (execEntry) {
        execBytes = await execEntry.async('uint8array');
      }

      let archMismatch = false;
      let archMismatchDetail = '';
      const frameworkDylibs = injectedDylibs.filter(d => d.mode === 'framework');

      if (execBytes) {
        const mainMach = this.inspectMachO(execBytes);
        for (const dylib of frameworkDylibs) {
          const dylibMach = this.inspectMachO(dylib.bytes);
          if (mainMach.is64 && !dylibMach.is64 && !dylibMach.isFat) {
            archMismatch = true;
            archMismatchDetail += `• Tệp "${dylib.name}" là 32-bit (ARMv7), không tương thích với Main Binary 64-bit (ARM64).\n`;
          }
        }
      }
      ...
      if (archMismatch) {
        checks.push({
          id: 'arch_mismatch',
          name: 'Tương Thích Kiến Trúc CPU (ARM64 / 64-bit)',
          passed: false,
          severity: 'critical',
          message: '❌ LỆCH KIẾN TRÚC CPU: Binary và Dylib không cùng kiến trúc!',
          detail: archMismatchDetail + 'Lỗi này sẽ gây văng ngay lập tức: dyld: no suitable image found (wrong architecture).',
          recommendation: 'Cần build lại tệp dylib với kiến trúc target là arm64 hoặc arm64e.',
          autoFixable: false
        });
      } else {
        passedCount++;
        checks.push({
          id: 'arch_mismatch',
          name: 'Tương Thích Kiến Trúc CPU (ARM64)',
          passed: true,
          severity: 'pass',
          message: '🟢 ĐẠT CHUẨN: Toàn bộ Binary và Dylib đồng nhất chuẩn 64-bit ARM64',
          detail: `Main Executable (${appInfo.arch}) hoàn toàn tương thích với các Dylib tiêm vào.`,
          autoFixable: false
        });
      }

      // 3. MISSING DEPENDENCY / CYDIASUBSTRATE / ELLEKIT TEST
      let missingSubstrate = false;
      let missingList = [];
      if (frameworkDylibs.length > 0) {
        frameworkDylibs.forEach(d => {
          const deps = this.parseMachODependencies(d.bytes);
          deps.forEach(dep => {
            if (dep.path.includes('CydiaSubstrate') || dep.path.includes('libsubstrate') || dep.path.includes('ElleKit') || dep.path.includes('libhooker')) {
              // Check if that framework/dylib is present in IPA
              const targetName = dep.path.split('/').pop();
              const inZip = Object.keys(zip.files).some(k => k.toLowerCase().includes(targetName.toLowerCase()));
              const inInjected = injectedDylibs.some(x => x.name.toLowerCase().includes(targetName.toLowerCase()));
              if (!inZip && !inInjected) {
                missingSubstrate = true;
                missingList.push(`• Dylib "${d.name}" yêu cầu "${dep.path}" nhưng gói IPA chưa có!`);
              }
            }
          });
        });
      ...
      if (missingSubstrate) {
        checks.push({
          id: 'missing_substrate',
          name: 'Kiểm Tra Phụ Thuộc Hook (Substrate / ElleKit)',
          passed: false,
          severity: 'critical',
          message: '⚠️ CẢNH BÁO: Dylib yêu cầu thư viện Substrate/Hook nhưng IPA chưa có!',
          detail: missingList.join('\n') + '\nNếu không có Substrate, app sẽ bị văng (Library not loaded) khi gọi hàm hook.',
          recommendation: 'Hệ thống có thể tự động nhúng CydiaSubstrate stub wrapper để ngăn chặn văng app.',
          autoFixable: true
        });
      } else {
        passedCount++;
        checks.push({
          ...
          passed: true,
          severity: 'pass',
          message: '🟢 ĐẠT CHUẨN: Không phát hiện thiếu thư viện phụ thuộc Substrate',
          detail: 'Tất cả liên kết động (Dynamic Links) đều hợp lệ.',
          autoFixable: false
        });
      }

      // 4. RPATH & FRAMEWORKS FOLDER INTEGRITY CHECK
      let rpathMissing = false;
      if (execBytes && frameworkDylibs.length > 0) {
        const deps = this.parseMachODependencies(execBytes);
        const hasRpath = deps.some(d => d.type === 'LC_RPATH' && (d.path.includes('@executable_path/Frameworks') || d.path.includes('@loader_path/Frameworks') || d.path.includes('@rpath')));
        if (!hasRpath && !options.autoInjectRpath) {
          rpathMissing = true;
        }
      }
      ...
      if (rpathMissing) {
        checks.push({
          id: 'rpath_missing',
          name: 'Kiểm Tra Cấu Trúc @rpath & Frameworks',
          passed: false,
          severity: 'warning',
          message: '⚠️ Cảnh báo: Executable chính chưa có cờ LC_RPATH chỉ định thư mục Frameworks!',
          detail: 'Dylib nhúng vào có thể không tìm thấy đường dẫn load khi khởi chạy.',
          recommendation: 'Bật tùy chọn "Tự động nạp qua @rpath/Frameworks" để tự động vá binary.',
          autoFixable: true
        });
      } else {
        passedCount++;
        checks.push({
          ...
          passed: true,
          severity: 'pass',
          message: '🟢 ĐẠT CHUẨN: Cấu hình @rpath và thư mục nạp Frameworks chuẩn Sideloadly',
          detail: 'Đường dẫn nạp thư viện @rpath/Frameworks đã được cấu hình tối ưu.',
          autoFixable: false
        });
      }

      // 5. INFO.PLIST CFBUNDLEEXECUTABLE INTEGRITY (Case Sensitivity Check)
      const exactExecFile = Object.keys(zip.files).find(k => k === `${appDir}${appInfo.executableName}`);
      if (!exactExecFile) {
        // Check case insensitive
        const caseInsensitive = Object.keys(zip.files).find(k => k.toLowerCase() === `${appDir}${appInfo.executableName}`.toLowerCase());
        if (caseInsensitive) {
          checks.push({
            id: 'plist_exec_case',
            name: 'Kiểm Tra Tên Executable (Info.plist vs Binary File)',
            passed: false,
            severity: 'critical',
            message: '❌ LỆCH CHỮ HOA/THƯỜNG: CFBundleExecutable không khớp tên file thực thi!',
            detail: `Info.plist ghi "${appInfo.executableName}", nhưng file thực tế là "${caseInsensitive.split('/').pop()}". iOS phân biệt hoa thường nên sẽ báo "App executable not found" và văng!`,
            recommendation: 'Tự động sửa lại CFBundleExecutable trong Info.plist cho khớp 100%.',
            autoFixable: true
          });
        } else {
          checks.push({
            id: 'plist_exec_case',
            name: 'Kiểm Tra Tên Executable trong Info.plist',
            passed: false,
            severity: 'critical',
            message: '❌ Không tìm thấy tệp thực thi chính trong gói ứng dụng!',
            detail: `Không tìm thấy file "${appInfo.executableName}" trong Payload/*.app.`,
            autoFixable: false
          });
        }
      } else {
        ...
        checks.push({
          id: 'plist_exec_case',
          name: 'Kiểm Tra Khớp Tên Executable (Info.plist)',
          passed: true,
          severity: 'pass',
          message: '🟢 ĐẠT CHUẨN: CFBundleExecutable khớp 100% với Binary thực tế',
          detail: `Tệp thực thi: "${appInfo.executableName}" tồn tại hợp lệ.`,
          autoFixable: false
        });
      }

      // 6. CODE SIGNING & CERTIFICATE INTEGRITY CHECK
      const isExportOnly = options.signMode === 'export';
      const isCustomCert = options.signMode === 'custom';
      if (isCustomCert && (!options.p12Bytes || !options.provBytes)) {
        checks.push({
          id: 'cert_check',
          name: 'Kiểm Tra Chứng Chỉ & MobileProvision (.p12)',
          passed: false,
          severity: 'warning',
          message: '⚠️ Bạn chọn ký chứng chỉ riêng nhưng chưa tải lên đủ tệp .p12 và .mobileprovision!',
          detail: 'Vui lòng nạp đủ cả 2 tệp để hệ thống ký số chính xác.',
          autoFixable: false
        });
      } else {
        passedCount++;
        checks.push({
          id: 'cert_check',
          name: 'Kiểm Tra Chế Độ Ký Số & Chữ Ký',
          passed: true,
          severity: 'pass',
          message: isExportOnly
            ? '🟢 ĐẠT CHUẨN: Chế độ Export IPA sạch (Tương thích 100% TrollStore/AltStore/Feather)'
            : '🟢 ĐẠT CHUẨN: Cấu hình ký số Code Signing hợp lệ',
          detail: isExportOnly ? 'Đã làm sạch chữ ký cũ để tránh xung đột Entitlements.' : 'Cấu hình ký số sẵn sàng.',
          autoFixable: false
        });
      }

      const totalChecks = checks.length;
      const isCrashProof = !checks.some(c => c.severity === 'critical');
...
\t<true/>
     ...
     */
    static createMachODylib(options = {}) {
      let name = (options.name || 'CustomTweak.dylib').trim();
      if (!name.endsWith('.dylib')) name += '.dylib';

      const installNameScheme = options.installNameScheme || 'rpath';
      let installNamePath = `@rpath/Frameworks/${name}`;
      if (installNameScheme === 'exec') installNamePath = `@executable_path/${name}`;
      if (installNameScheme === 'loader') installNamePath = `@loader_path/${name}`;

      const watermark = options.watermark || `CipherVault Pro Mach-O ARM64 Dylib (${name})`;

      // Total size 4096 bytes (standard page)
      const buffer = new Uint8Array(4096);
      const view = new DataView(buffer.buffer);

      const writeString = (str, offset, maxLen = str.length) => {
      ...
      view.setUint32(0, 0xFEEDFACF, true); // Magic 64-bit LE
      view.setUint32(4, 0x0100000C, true); // CPU_TYPE_ARM64
      view.setUint32(8, 0x00000000, true); // CPU_SUBTYPE_ARM64_ALL
      view.setUint32(12, 0x00000006, true); // MH_DYLIB
      view.setUint32(24, 0x00100085, true); // flags (MH_NOUNDEFS | MH_DYLDLINK | MH_TWOLEVEL | MH_WEAK_DEFINES)
      view.setUint32(28, 0x00000000, true); // reserved

      let cmdOffset = 32;
      let numCmds = 0;

      // Command 1: LC_ID_DYLIB (0x0D)
      const idPathBytes = new TextEncoder().encode(installNamePath + '\0');
      const idCmdSize = Math.ceil((24 + idPathBytes.length) / 8) * 8;
      view.setUint32(cmdOffset, 0x0000000D, true); // LC_ID_DYLIB
      view.setUint32(cmdOffset + 4, idCmdSize, true);
      view.setUint32(cmdOffset + 8, 24, true); // name offset
      view.setUint32(cmdOffset + 12, 1, true); // timestamp
      view.setUint32(cmdOffset + 16, 0x00010000, true); // current version 1.0.0
      view.setUint32(cmdOffset + 20, 0x00010000, true); // compat version 1.0.0
      buffer.set(idPathBytes, cmdOffset + 24);
      cmdOffset += idCmdSize;
      numCmds++;

      // Command 2: LC_LOAD_DYLIB (/usr/lib/libSystem.B.dylib) (0x0C)
      const libSysPath = new TextEncoder().encode('/usr/lib/libSystem.B.dylib\0');
      const loadCmdSize = Math.ceil((24 + libSysPath.length) / 8) * 8;
      view.setUint32(cmdOffset, 0x0000000C, true); // LC_LOAD_DYLIB
      view.setUint32(cmdOffset + 4, loadCmdSize, true);
      view.setUint32(cmdOffset + 8, 24, true);
      view.setUint32(cmdOffset + 12, 2, true);
      view.setUint32(cmdOffset + 16, 0x00010000, true);
      view.setUint32(cmdOffset + 20, 0x00010000, true);
      buffer.set(libSysPath, cmdOffset + 24);
      cmdOffset += loadCmdSize;
      numCmds++;

      // Command 3: LC_UUID (0x1B)
      view.setUint32(cmdOffset, 0x0000001B, true); // LC_UUID
      view.setUint32(cmdOffset + 4, 24, true);
      const uuidBytes = new Uint8Array(16);
      window.crypto.getRandomValues(uuidBytes);
      buffer.set(uuidBytes, cmdOffset + 8);
      cmdOffset += 24;
      numCmds++;

      // Command 4: LC_SEGMENT_64 (__TEXT) (0x19)
      const cstringOffset = 512;
      const cstringData = new TextEncoder().encode(watermark + '\0');
      buffer.set(cstringData, cstringOffset);

      const segCmdSize = 72 + 80;
      view.setUint32(cmdOffset, 0x00000019, true); // LC_SEGMENT_64
      view.setUint32(cmdOffset + 4, segCmdSize, true);
      writeString('__TEXT', cmdOffset + 8, 16);
      view.setBigUint64(cmdOffset + 24, 0x0n, true); // vmaddr
      view.setBigUint64(cmdOffset + 32, 4096n, true); // vmsize
      view.setBigUint64(cmdOffset + 40, 0x0n, true); // fileoff
      view.setBigUint64(cmdOffset + 48, 4096n, true); // filesize
      view.setUint32(cmdOffset + 56, 0x7, true); // maxprot rwx
      view.setUint32(cmdOffset + 60, 0x5, true); // initprot r-x
      view.setUint32(cmdOffset + 64, 1, true); // nsects = 1
      view.setUint32(cmdOffset + 68, 0, true); // flags

      // Section 1: __cstring inside __TEXT
      const sectOffset = cmdOffset + 72;
      writeString('__cstring', sectOffset, 16);
      writeString('__TEXT', sectOffset + 16, 16);
      view.setBigUint64(sectOffset + 32, BigInt(cstringOffset), true); // addr
      view.setBigUint64(sectOffset + 40, BigInt(cstringData.length), true); // size
      view.setUint32(sectOffset + 48, cstringOffset, true); // offset
      view.setUint32(sectOffset + 52, 0, true); // align
      view.setUint32(sectOffset + 56, 0, true); // reloff
      view.setUint32(sectOffset + 60, 0, true); // nreloc
      view.setUint32(sectOffset + 64, 0x00000002, true); // S_CSTRING_LITERALS

      cmdOffset += segCmdSize;
      numCmds++;

      // Update Header
      view.setUint32(16, numCmds, true);
      view.setUint32(20, cmdOffset - 32, true);

      return buffer;
    }

    /**
     * Generate bulk Mach-O .dylib files based on pattern or custom name list
     */
    static generateBulkDylibs(options = {}) {
      const mode = options.mode || 'pattern';
      const watermark = (options.watermark || 'CipherVault Custom Dylib').trim();
      const installNameScheme = options.installNameScheme || 'rpath';
      const results = [];

      if (mode === 'list' && options.namesList && options.namesList.length > 0) {
        const uniqueNames = Array.from(new Set(options.namesList.map(n => n.trim()).filter(Boolean)));
        uniqueNames.forEach(rawName => {
          let name = rawName;
          if (!name.endsWith('.dylib')) name += '.dylib';
          const bytes = this.createMachODylib({
            name,
            installNameScheme,
        ...
        const count = Math.min(Math.max(parseInt(options.count, 10) || 5, 1), 100);
        const prefix = (options.prefix || 'TweakPlugin_').trim();
        for (let i = 1; i <= count; i++) {
          const padNum = String(i).padStart(2, '0');
          const name = `${prefix}${padNum}.dylib`;
          const bytes = this.createMachODylib({
            name,
            installNameScheme,
            watermark: `${watermark} [${name}]`
  ...
  function initIpaStudioModule() {
    ...
    async function loadIpaFile(file) {
      ...
      try {
        ...
        updateDepTargetOptions();
        runAntiCrashDiagnosticUI();
        sfx.success();
    ...
    // Dylib Generator Studio
    const toggleDylibGenHeader = document.getElementById('toggleDylibGenHeader');
    const toggleDylibGenBtn = document.getElementById('toggleDylibGenBtn');
    const dylibGenPanelBody = document.getElementById('dylibGenPanelBody');
    const dylibGenChevron = document.getElementById('dylibGenChevron');
    const dylibGenModePattern = document.getElementById('dylibGenModePattern');
    const dylibGenModeList = document.getElementById('dylibGenModeList');
    const dylibGenPatternControls = document.getElementById('dylibGenPatternControls');
    const dylibGenListControls = document.getElementById('dylibGenListControls');
    const dylibGenPrefix = document.getElementById('dylibGenPrefix');
    const dylibGenCount = document.getElementById('dylibGenCount');
    const dylibGenNamesTextarea = document.getElementById('dylibGenNamesTextarea');
    const dylibGenInstallName = document.getElementById('dylibGenInstallName');
    const dylibGenWatermark = document.getElementById('dylibGenWatermark');
    const dylibGenDownloadZipBtn = document.getElementById('dylibGenDownloadZipBtn');
    const dylibGenInjectDirectBtn = document.getElementById('dylibGenInjectDirectBtn');
    const dylibCountQuickBtns = document.querySelectorAll('.dylib-count-quick');

    const toggleDylibGen = () => {
      sfx.click();
      const isOpen = dylibGenPanelBody.style.display === 'block';
      dylibGenPanelBody.style.display = isOpen ? 'none' : 'block';
      if (dylibGenChevron) {
        dylibGenChevron.className = isOpen ? 'fa-solid fa-chevron-down' : 'fa-solid fa-chevron-up';
      }
    };

    if (toggleDylibGenHeader) toggleDylibGenHeader.addEventListener('click', toggleDylibGen);
    if (toggleDylibGenBtn) toggleDylibGenBtn.addEventListener('click', (e) => { e.stopPropagation(); toggleDylibGen(); });

    if (dylibGenModePattern && dylibGenModeList) {
      const updateDylibGenMode = () => {
        const isPattern = dylibGenModePattern.checked;
        if (dylibGenPatternControls) dylibGenPatternControls.style.display = isPattern ? 'grid' : 'none';
        if (dylibGenListControls) dylibGenListControls.style.display = isPattern ? 'none' : 'block';
      };
      dylibGenModePattern.addEventListener('change', updateDylibGenMode);
      dylibGenModeList.addEventListener('change', updateDylibGenMode);
    }

    if (dylibCountQuickBtns && dylibCountQuickBtns.length > 0) {
      dylibCountQuickBtns.forEach(btn => {
        btn.addEventListener('click', () => {
          sfx.click();
          dylibCountQuickBtns.forEach(b => {
          ...
          if (dylibGenCount) {
            dylibGenCount.value = btn.getAttribute('data-count');
          }
        });
      });
    }

    const collectDylibGenOptions = () => {
      const isPattern = dylibGenModePattern ? dylibGenModePattern.checked : true;
      const count = parseInt(dylibGenCount?.value, 10) || 5;
      const prefix = dylibGenPrefix?.value || 'TweakPlugin_';
      const watermark = dylibGenWatermark?.value || 'CipherVault Custom Dylib Module';
      const installNameScheme = dylibGenInstallName?.value || 'rpath';
      const namesList = dylibGenNamesTextarea?.value
        ? dylibGenNamesTextarea.value.split('\n').map(s => s.trim()).filter(Boolean)
        : [];

      return {
        mode: isPattern ? 'pattern' : 'list',
        count,
        prefix,
        watermark,
        installNameScheme,
    ...
    if (dylibGenInjectDirectBtn) {
      dylibGenInjectDirectBtn.addEventListener('click', () => {
        if (!enforceOperationGate()) return;
        sfx.click();
        try {
          const opts = collectDylibGenOptions();
          const generated = IpaStudioEngine.generateBulkDylibs(opts);
          if (generated.length === 0) {
            showToast('Không có tệp .dylib nào được sinh!', 'warning');
            return;
          }

          let addedCount = 0;
          generated.forEach(d => {
            const exists = injectedDylibs.some(existing => existing.name.toLowerCase() === d.name.toLowerCase());
            if (!exists) {
              injectedDylibs.push(d);
              addedCount++;
            }
          });
          ...
          renderDylibList();
          sfx.success();
          showToast(`🎉 Đã sinh và thêm ${addedCount} tệp .dylib chuẩn Mach-O ARM64 vào danh sách tiêm!`, 'success');
          AuditLogger.log('Sinh Dylib Tiêm Trực Tiếp', `${addedCount} tệp .dylib`, addedCount * 4096, 'Thành công');
        } catch (err) {
          console.error('Inject generated dylib error:', err);
          sfx.error();
          showToast('Lỗi sinh dylib: ' + err.message, 'error');
        }
      });
    }

    if (dylibGenDownloadZipBtn) {
      dylibGenDownloadZipBtn.addEventListener('click', async () => {
        if (!enforceOperationGate()) return;
        sfx.click();
        try {
          const opts = collectDylibGenOptions();
          const generated = IpaStudioEngine.generateBulkDylibs(opts);
          if (generated.length === 0) {
            showToast('Không có tệp .dylib nào để tải!', 'warning');
            return;
          }

          showToast(`⚡ Đang đóng gói ${generated.length} tệp .dylib vào file ZIP...`, 'info');
          const zip = new JSZip();
          let totalBytes = 0;
          generated.forEach(d => {
          ...
          const zipName = `Dylibs_Pack_${generated.length}Files_${Date.now().toString().slice(-4)}.zip`;
          const a = document.createElement('a');
          a.href = downloadUrl;
          a.download = zipName;
          ...
          sfx.success();
          showToast(`🎉 Đã tải về thành công gói ${generated.length} tệp .dylib (${formatBytes(totalBytes)})!`, 'success');
          AuditLogger.log('Tải Gói Dylib ZIP', zipName, zipBlob.size, 'Thành công');
        } catch (err) {
          console.error('Export dylib zip error:', err);
          sfx.error();
          showToast('Lỗi khi tải gói dylib: ' + err.message, 'error');
        }
      });
    }

    // ══════════════════════════════════════════════════════════════════════════
    //   MACH-O DEPENDENCY INSPECTOR & LIVE MODIFIER (install_name_tool Clone)
    // ══════════════════════════════════════════════════════════════════════════
    const toggleDepInspectorHeader = document.getElementById('toggleDepInspectorHeader');
    const toggleDepInspectorBtn = document.getElementById('toggleDepInspectorBtn');
    const depInspectorPanelBody = document.getElementById('depInspectorPanelBody');
    const depInspectorChevron = document.getElementById('depInspectorChevron');
    const depTargetSelect = document.getElementById('depTargetSelect');
    const refreshDepListBtn = document.getElementById('refreshDepListBtn');
    const depTableBody = document.getElementById('depTableBody');
    const depOldPathInput = document.getElementById('depOldPathInput');
    const depNewPathInput = document.getElementById('depNewPathInput');
    const applyDepChangeBtn = document.getElementById('applyDepChangeBtn');
    const quickAddRpathBtn = document.getElementById('quickAddRpathBtn');
    const autoFixAllRpathsBtn = document.getElementById('autoFixAllRpathsBtn');

    const toggleDepInspector = () => {
      sfx.click();
      const isOpen = depInspectorPanelBody.style.display === 'block';
      depInspectorPanelBody.style.display = isOpen ? 'none' : 'block';
      if (depInspectorChevron) {
        depInspectorChevron.className = isOpen ? 'fa-solid fa-chevron-down' : 'fa-solid fa-chevron-up';
      }
      if (!isOpen) updateDepTargetOptions();
    };

    if (toggleDepInspectorHeader) toggleDepInspectorHeader.addEventListener('click', toggleDepInspector);
    if (toggleDepInspectorBtn) toggleDepInspectorBtn.addEventListener('click', (e) => { e.stopPropagation(); toggleDepInspector(); });

    function updateDepTargetOptions() {
      if (!depTargetSelect) return;
      depTargetSelect.innerHTML = '<option value="main">Main Executable (App Binary)</option>';
      injectedDylibs.forEach((d, idx) => {
        if (d.mode === 'framework') {
          const opt = document.createElement('option');
          opt.value = `dylib_${idx}`;
          opt.textContent = `Dylib: ${d.name}`;
    ...
    async function getSelectedTargetBytes() {
      if (!loadedIpa) return null;
      const targetVal = depTargetSelect ? depTargetSelect.value : 'main';
      if (targetVal === 'main') {
        const execEntry = loadedIpa.zip.file(`${loadedIpa.appDir}${loadedIpa.appInfo.executableName}`);
        if (execEntry) {
          return await execEntry.async('uint8array');
        }
      } else if (targetVal.startsWith('dylib_')) {
        const idx = parseInt(targetVal.split('_')[1], 10);
        if (injectedDylibs[idx]) {
          return injectedDylibs[idx].bytes;
        }
      }
      return null;
    }

    async function renderDepTable() {
      if (!depTableBody) return;
      if (!loadedIpa) {
        depTableBody.innerHTML = '<tr><td colspan="3" style="text-align:center;color:var(--text-muted);padding:14px;">Chưa nạp tệp IPA.</td></tr>';
        return;
      }

      const bytes = await getSelectedTargetBytes();
      if (!bytes) {
        depTableBody.innerHTML = '<tr><td colspan="3" style="text-align:center;color:var(--text-muted);padding:14px;">Không thể đọc nhị phân Mach-O của mục tiêu đã chọn.</td></tr>';
        return;
      }

      const deps = IpaStudioEngine.parseMachODependencies(bytes);
      if (deps.length === 0) {
        depTableBody.innerHTML = '<tr><td colspan="3" style="text-align:center;color:var(--text-muted);padding:14px;">Không tìm thấy Load Commands phụ thuộc (LC_LOAD_DYLIB / LC_RPATH).</td></tr>';
        return;
      }
      ...
      deps.forEach(dep => {
        ...
        tr.innerHTML = `
          <td><span class="dep-type-badge ${dep.badgeClass}">${dep.type}</span></td>
          <td class="font-mono" style="word-break:break-all;color:${dep.path.startsWith('@rpath') ? 'var(--accent-cyan)' : '#e2e8f0'};font-size:0.75rem;">${dep.path}</td>
          <td>
            <button type="button" class="mini-btn pick-dep-btn" data-path="${dep.path}" title="Chọn để sửa đường dẫn này" style="font-size:0.72rem;padding:2px 6px;">
      ...
      depTableBody.querySelectorAll('.pick-dep-btn').forEach(btn => {
        btn.addEventListener('click', () => {
          sfx.click();
          const p = btn.getAttribute('data-path');
          if (depOldPathInput) depOldPathInput.value = p;
          if (depNewPathInput) {
            const fileName = p.split('/').pop();
            depNewPathInput.value = `@rpath/Frameworks/${fileName}`;
          }
          showToast(`Đã chọn phụ thuộc: ${p}`, 'info');
        });
      });
    }

    if (depTargetSelect) depTargetSelect.addEventListener('change', () => { sfx.click(); renderDepTable(); });
    if (refreshDepListBtn) refreshDepListBtn.addEventListener('click', () => { sfx.click(); renderDepTable(); showToast('Đã làm mới danh sách phụ thuộc!', 'info'); });

    if (applyDepChangeBtn) {
      applyDepChangeBtn.addEventListener('click', async () => {
        if (!enforceOperationGate()) return;
        const oldP = (depOldPathInput?.value || '').trim();
        const newP = (depNewPathInput?.value || '').trim();
        if (!oldP || !newP) {
          showToast('Vui lòng nhập đầy đủ đường dẫn cũ và đường dẫn mới!', 'error');
          return;
        }

        const bytes = await getSelectedTargetBytes();
        if (!bytes) {
          showToast('Không tìm thấy dữ liệu nhị phân mục tiêu!', 'error');
          return;
        }

        const res = IpaStudioEngine.modifyMachODependency(bytes, oldP, newP);
        if (res) {
          sfx.success();
          renderDepTable();
          runAntiCrashDiagnosticUI();
          showToast(`🎉 Đã sửa đổi đường dẫn phụ thuộc thành công:\n"${oldP}" ➔ "${newP}"`, 'success');
          AuditLogger.log('Sửa Mach-O Dependency', `${oldP} ➔ ${newP}`, 0, 'Thành công');
        } else {
          sfx.error();
          showToast('Không thể sửa đường dẫn (quá dài hoặc không tìm thấy khớp)!', 'error');
        }
      });
    }

    if (autoFixAllRpathsBtn) {
      autoFixAllRpathsBtn.addEventListener('click', async () => {
        if (!enforceOperationGate()) return;
        sfx.click();
        let fixedCount = 0;
        injectedDylibs.forEach(d => {
          if (d.mode === 'framework') {
            const deps = IpaStudioEngine.parseMachODependencies(d.bytes);
            deps.forEach(dep => {
              if (dep.path.includes('/Library/MobileSubstrate/') || dep.path.includes('/Library/Frameworks/')) {
                const fName = dep.path.split('/').pop();
                const newP = `@rpath/Frameworks/${fName}`;
                if (IpaStudioEngine.modifyMachODependency(d.bytes, dep.path, newP)) {
                  fixedCount++;
                }
              }
            });
        ...
        runAntiCrashDiagnosticUI();
        sfx.success();
        showToast(`🎉 Đã tự động vá ${fixedCount} liên kết phụ thuộc sang chuẩn @rpath/Frameworks!`, 'success');
      });
    }

    // ══════════════════════════════════════════════════════════════════════════
    //   PRE-FLIGHT ANTI-CRASH DEEP DIAGNOSTIC HUD & REPORT ENGINE
    // ══════════════════════════════════════════════════════════════════════════
    const antiCrashDiagCard = document.getElementById('antiCrashDiagCard');
    const crashDiagMainTitle = document.getElementById('crashDiagMainTitle');
    const crashDiagStatusBadge = document.getElementById('crashDiagStatusBadge');
    const runAntiCrashDiagBtn = document.getElementById('runAntiCrashDiagBtn');
    const autoFixCrashRisksBtn = document.getElementById('autoFixCrashRisksBtn');
    const crashChecklistGrid = document.getElementById('crashChecklistGrid');

    async function runAntiCrashDiagnosticUI() {
      if (!loadedIpa || !crashChecklistGrid) return;

      const signMode = signExportRadio && signExportRadio.checked
        ? 'export'
        : (signCustomRadio && signCustomRadio.checked ? 'custom' : 'adhoc');

      const diag = await IpaStudioEngine.runFullAntiCrashDiagnostic(loadedIpa, injectedDylibs, {
        signMode,
        p12Bytes,
        provBytes,
        autoInjectRpath: document.getElementById('ipaAutoInjectRpath')?.checked
      });

      crashChecklistGrid.innerHTML = '';
      let hasAutoFixable = false;

      diag.checks.forEach(c => {
        const item = document.createElement('div');
        const passClass = c.passed ? 'passed' : (c.severity === 'critical' ? 'critical' : 'warning');
        const iconClass = c.passed ? 'fa-circle-check pass' : (c.severity === 'critical' ? 'fa-circle-xmark fail' : 'fa-triangle-exclamation warn');
        item.className = `crash-check-item ${passClass}`;

        const recHtml = c.recommendation
          ? `<div style="font-size:0.72rem;color:#67e8f9;margin-top:4px;"><i class="fa-solid fa-lightbulb"></i> <strong>Giải pháp:</strong> ${c.recommendation}</div>`
          : '';

        item.innerHTML = `
          <i class="fa-solid ${iconClass} crash-item-icon ${c.passed ? 'pass' : (c.severity === 'critical' ? 'fail' : 'warn')}"></i>
          <div class="crash-item-content">
            <span class="crash-item-name">${c.name}</span>
            <span class="crash-item-desc">${c.message}</span>
            <span class="crash-item-desc" style="font-size:0.7rem;margin-top:2px;">${c.detail}</span>
            ${recHtml}
          </div>
        `;
        crashChecklistGrid.appendChild(item);

        if (!c.passed && c.autoFixable) hasAutoFixable = true;
      });

      if (antiCrashDiagCard) {
        antiCrashDiagCard.className = `crash-diag-card ${diag.isCrashProof ? 'status-pass' : 'status-fail'}`;
      }

      if (crashDiagStatusBadge) {
        if (diag.isCrashProof) {
          crashDiagStatusBadge.innerHTML = `<span class="pulse-dot" style="background:#10b981;box-shadow:0 0 8px #10b981;"></span> 100% CRASH-PROOF READY (${diag.score}%)`;
          crashDiagStatusBadge.style.color = '#34d399';
          crashDiagStatusBadge.style.borderColor = 'rgba(16, 185, 129, 0.4)';
          crashDiagStatusBadge.style.background = 'rgba(16, 185, 129, 0.1)';
        } else {
          crashDiagStatusBadge.innerHTML = `<span class="pulse-dot" style="background:#ef4444;box-shadow:0 0 8px #ef4444;"></span> PHÁT HIỆN NGUY CƠ VĂNG APP`;
          crashDiagStatusBadge.style.color = '#f87171';
          crashDiagStatusBadge.style.borderColor = 'rgba(239, 68, 68, 0.4)';
          crashDiagStatusBadge.style.background = 'rgba(239, 68, 68, 0.1)';
        }
      }

      if (crashDiagMainTitle) {
        if (diag.isCrashProof) {
          crashDiagMainTitle.innerHTML = `<i class="fa-solid fa-circle-check" style="color:#10b981;"></i> <span style="color:#34d399;">Cấu Trúc Đạt Chuẩn Hoạt Động 100% — Không Lo Bị Văng!</span>`;
        } else {
          crashDiagMainTitle.innerHTML = `<i class="fa-solid fa-triangle-exclamation" style="color:#ef4444;"></i> <span style="color:#f87171;">Cảnh Báo: Phát Hiện Yếu Tố Gây Văng Ứng Dụng (High Crash Risk)</span>`;
        }
      }

      if (autoFixCrashRisksBtn) {
        autoFixCrashRisksBtn.style.display = hasAutoFixable ? 'inline-flex' : 'none';
      }

      return diag;
    }

    if (runAntiCrashDiagBtn) {
      runAntiCrashDiagBtn.addEventListener('click', async () => {
        sfx.click();
        showToast('🔍 Đang chẩn đoán chuyên sâu cấu trúc binary...', 'info');
        const diag = await runAntiCrashDiagnosticUI();
        sfx.success();
        if (diag.isCrashProof) {
          showToast('🎉 Kết quả: Cấu trúc đạt chuẩn 100%! Bản build an toàn tuyệt đối.', 'success');
        } else {
          showToast('⚠️ Phát hiện nguy cơ gây văng app! Xem chi tiết ở bảng chẩn đoán bên dưới.', 'warning');
        }
      });
    }

    if (autoFixCrashRisksBtn) {
      autoFixCrashRisksBtn.addEventListener('click', async () => {
        if (!enforceOperationGate()) return;
        sfx.click();
        showToast('⚡ Đang tự động sửa lỗi cấu trúc và vá liên kết...', 'info');

        // 1. Auto fix Info.plist executable case
        if (loadedIpa) {
          const exact = Object.keys(loadedIpa.zip.files).find(k => k.toLowerCase() === `${loadedIpa.appDir}${loadedIpa.appInfo.executableName}`.toLowerCase());
          if (exact) {
            loadedIpa.appInfo.executableName = exact.split('/').pop();
          }
        }

        // 2. Enable rpath injection
        const autoRpathChk = document.getElementById('ipaAutoInjectRpath');
        if (autoRpathChk) autoRpathChk.checked = true;

        // 3. Auto inject a lightweight CydiaSubstrate stub dylib if missing
        const hasSubstrateInInjected = injectedDylibs.some(d => d.name.toLowerCase().includes('substrate'));
        if (!hasSubstrateInInjected) {
          const substrateStub = IpaStudioEngine.createMachODylib({
            name: 'CydiaSubstrate.dylib',
            installNameScheme: 'rpath',
            watermark: 'CipherVault Hook Engine Substrate Stub'
          });
          injectedDylibs.push({
            name: 'CydiaSubstrate.dylib',
            bytes: substrateStub,
            size: substrateStub.byteLength,
            mode: 'framework',
            obfuscate: false,
            warning: null
          });
        ...
        await runAntiCrashDiagnosticUI();
        sfx.success();
        showToast('🎉 Đã tự động vá các lỗi có thể sửa! Cấu trúc đã được tối ưu hóa chống văng.', 'success');
      });
    }
    ...
    startBuildBtn.addEventListener('click', async () => {
      ...
      // Run Pre-Flight Anti-Crash Diagnostic check
      const diag = await IpaStudioEngine.runFullAntiCrashDiagnostic(loadedIpa, injectedDylibs, {
        signMode: isExportOnly ? 'export' : (isCustomCert ? 'custom' : 'adhoc'),
        p12Bytes,
        provBytes,
        autoInjectRpath: document.getElementById('ipaAutoInjectRpath')?.checked
      });
      ...
      if (!diag.isCrashProof) {
        const failMsgs = diag.checks.filter(c => !c.passed && c.severity === 'critical').map(c => '• ' + c.name + ': ' + c.message).join('\n');
        const proceed = confirm(
          `⚠️ CẢNH BÁO NGUY HIỂM — NGUY CƠ VĂNG ỨNG DỤNG KHI CÀI ĐẶT:\n\n${failMsgs}\n\nỨng dụng có thể bị iOS tắt ngay khi vừa mở. Bạn có chắc chắn muốn bỏ qua cảnh báo và tiếp tục đóng gói không?`
        );
        if (!proceed) return;
      }
  ...
  const ROOT_SIG = {
    ...
    PIN_HASH: 'd7a39e5bf84461685d6fa7345fe1e7016b92cd8c2f4ad535075f4aa6b9d35cdd' // sha256(210209)
  };
  ...
  // ==========================================================================
  // OPERATION ACCESS GATEKEEPER (MANDATORY AUTH & ANTI-SHARE ENGINE)
  // - Maintenance Mode: Blocks everyone except Root Admin
  // - Admin & Seller: VIP Bypass
  // - Guest User: STRICTLY BLOCKED, must Login or Register
  // - Registered User: Validated subscription
  // ==========================================================================
  function enforceOperationGate() {
    // 0. Maintenance Mode Check
    const settings = AppSettings.get();
    if (settings.maintenanceMode && currentOsSession.role !== 'admin') {
      sfx.error();
      showToast('⚠️ Máy chủ hiện đang trong chế độ BẢO TRÌ NÂNG CẤP! Vui lòng quay lại sau ít phút.', 'error');
      return false;
    }
    ...
    if (curUser.status === 'banned') {
      sfx.error();
      showToast('⛔ Tài khoản @' + curUser.username + ' đã bị khóa bởi Admin do vi phạm chính sách!', 'error');
      UserManager.logout();
      const authModal = document.getElementById('userAuthModal');
      if (authModal) authModal.style.display = 'flex';
      return false;
    ...
    // 4. Global Free Mode Check (if enabled by admin)
    if (settings.freeMode) {
      return true;
    }

    // 5. Check User Subscription Expiry
  ...
  class UserManager {
    ...
    static register(username, password, displayName) {
      ...
      if (settings.allowRegistrations === false) {
        return { success: false, reason: 'Hệ thống hiện đang tạm khóa đăng ký tài khoản mới!' };
      }
      ...
      // Mặc định KHÔNG cấp free dùng thử. Chỉ cấp vĩnh viễn nếu Admin BẬT tính năng "Tự Động Mở Free Khi Mới Đăng Ký"
      const autoFree = settings.autoFreeNewUsers === true;
      const newUser = {
        ...
        displayName: d,
        createdAt: new Date().toLocaleString(),
        activeKey: autoFree ? 'AUTO-FREE-' + Date.now().toString(36).toUpperCase() : null,
        activeKeyLabel: autoFree ? 'Miễn phí vĩnh viễn (Admin)' : null,
        subscriptionExpires: autoFree ? 9999999999999 : null,
        status: 'active',
        role: 'user',
    ...
    static login(username, password) {
      ...
      if (found.status === 'banned') {
        return { success: false, reason: 'Tài khoản này đã bị khóa bởi Admin do vi phạm chính sách!' };
      }
    ...
    static toggleUserStatus(username) {
      const users = this.getUsers();
      ...
      return { success: false, reason: 'Không tìm thấy user!' };
    }

    static grantUserTime(username, hours) {
      const users = this.getUsers();
      const user = users.find(x => x.username.toLowerCase() === username.toLowerCase());
      if (!user) return { success: false, reason: 'Không tìm thấy user!' };

      const now = Date.now();
      if (hours >= 999999) {
        user.subscriptionExpires = 9999999999999;
        user.activeKeyLabel = 'Vĩnh Viễn';
      } else {
        const msToAdd = hours * 3600000;
        if (user.subscriptionExpires && user.subscriptionExpires > now && user.subscriptionExpires < 9999999999999) {
          user.subscriptionExpires += msToAdd;
        } else {
          user.subscriptionExpires = now + msToAdd;
        }
        user.activeKeyLabel = `Admin Cấp (+${hours}h)`;
      }
      this.saveUsers(users);
      return { success: true, newExpiry: user.subscriptionExpires };
    }
    ...
    static deleteUser(username) {
      ...
      this.saveUsers(users);
      if (localStorage.getItem(this.SESSION_KEY) === username) {
        this.setCurrentUser(null);
      }
      return { success: true };
    }
    ...
    static activateKeyForUser(username, inputKey) {
      ...
      if (isLifetime) {
        ...
        // Chỉ cộng thêm vào thời gian hiện tại nếu user đang có KEY THỰC SỰ (không phải trial dùng thử)
        const isTrial = !user.activeKey || user.activeKey.startsWith('TRIAL-') || user.activeKey.startsWith('AUTO-FREE-');
        const hasRealActiveSubscription = !isTrial
          && user.subscriptionExpires
          && user.subscriptionExpires > now
          && user.subscriptionExpires < 9999999999999;

        if (hasRealActiveSubscription) {
          // Có key thật đang còn hạn → cộng thêm (stack)
          newExpiry = user.subscriptionExpires + durationMs;
        } else {
          // Trial hoặc hết hạn → tính mới hoàn toàn từ thời điểm kích hoạt
  ...
  class AppSettings {
    ...
    static getDefaults() {
      return {
        ...
        serverStartTimestamp: Date.now() - 259200000 // 3 days uptime
      };
    }
  ...
  function initAdminAndPromoSystem() {
    ...
    document.querySelectorAll('.os-nav-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if (targetTab === 'users') renderAdminUserList();
      });
    });

    // 8. Setup OS Live Clock & Server Health Daemon
    let simCpu = 12;
    let simRam = 48;
    let simPing = 16;
    ...
    setInterval(() => {
      ...
      const settings = AppSettings.get();
      const uptimeMs = now - (settings.serverStartTimestamp || (now - 259200000));
      const uptimeEl = document.getElementById('statServerUptime');
      if (uptimeEl) uptimeEl.textContent = `99.99% (${formatUptime(uptimeMs)})`;

      // Fluctuate simulated metrics smoothly
      simCpu = Math.max(5, Math.min(38, simCpu + (Math.random() * 4 - 2)));
      simRam = Math.max(35, Math.min(120, simRam + (Math.random() * 2 - 1)));
      simPing = Math.max(9, Math.min(32, Math.round(simPing + (Math.random() * 4 - 2))));

      const cpuText = document.getElementById('statCpuPercent');
      const cpuFill = document.getElementById('meterCpuFill');
      if (cpuText) cpuText.textContent = Math.round(simCpu) + '%';
      if (cpuFill) cpuFill.style.width = Math.round(simCpu) + '%';

      const ramText = document.getElementById('statRamUsage');
      const ramFill = document.getElementById('meterRamFill');
      if (ramText) ramText.textContent = `${Math.round(simRam)} MB / 2 GB`;
      if (ramFill) ramFill.style.width = Math.min(100, Math.round((simRam / 200) * 100)) + '%';

      const pingText = document.getElementById('statServerPing');
      const headerPingText = document.getElementById('headerServerPingText');
      if (pingText) pingText.textContent = `${simPing} ms`;
      if (headerPingText) headerPingText.textContent = `${simPing}ms`;

      const probeText = document.getElementById('statBlockedProbes');
      if (probeText) probeText.textContent = _CipherShield.getBlockedProbesCount();
    }, 1000);
    ...
    let currentAdminTargetUsername = null;
    const adminUserActionModal = document.getElementById('adminUserActionModal');
    const closeAdminUserActionModalBtn = document.getElementById('closeAdminUserActionModalBtn');
    const searchUserInput = document.getElementById('searchUserInput');
    const refreshUserListBtn = document.getElementById('refreshUserListBtn');

    if (searchUserInput) searchUserInput.addEventListener('input', () => renderAdminUserList());
    ...
    // Công tắc Auto-Free cho người dùng mới đăng ký
    const adminAutoFreeNewUsersToggle = document.getElementById('adminAutoFreeNewUsersToggle');
    if (adminAutoFreeNewUsersToggle) {
      adminAutoFreeNewUsersToggle.addEventListener('change', () => {
        sfx.click();
        const cur = AppSettings.get();
        cur.autoFreeNewUsers = adminAutoFreeNewUsersToggle.checked;
        AppSettings.save(cur);
        showToast(
          adminAutoFreeNewUsersToggle.checked
            ? '✅ Đã bật: Người dùng đăng ký mới sẽ được miễn phí VĨNH VIỄN tự động!'
            : '🔒 Đã tắt: Người dùng mới cần nạp Key bản quyền để sử dụng.',
          'info'
        );
      });
    }

    if (closeAdminUserActionModalBtn && adminUserActionModal) {
      closeAdminUserActionModalBtn.addEventListener('click', () => {
        adminUserActionModal.style.display = 'none';
      });
    }
    ...
    function renderAdminUserList() {
      ...
      if (q) {
        users = users.filter(u => u.username.toLowerCase().includes(q) || (u.displayName && u.displayName.toLowerCase().includes(q)));
      }

      if (users.length === 0) {
        tbody.innerHTML = '<tr><td colspan="8" style="text-align: center; color: var(--text-muted); padding: 20px;">Không có tài khoản người dùng nào.</td></tr>';
        return;
      }
      ...
      users.forEach(u => {
        ...
        const roleBadge = role === 'admin' ? '<span class="user-badge-role role-admin">ADMIN</span>' : role === 'vip' ? '<span class="user-badge-role role-vip">VIP</span>' : '<span class="user-badge-role role-user">MEMBER</span>';
        const statusBadge = isBanned
          ? '<span class="user-status-tag banned"><i class="fa-solid fa-ban"></i> Đã Khóa</span>'
          : '<span class="user-status-tag active"><i class="fa-solid fa-circle-check"></i> Hoạt Động</span>';

        let expiryStr = 'Chưa nạp key';
        if (u.subscriptionExpires >= 9999999999999) {
          expiryStr = '<span style="color:#fbbf24;font-weight:700;">Vĩnh viễn</span>';
        } else if (u.subscriptionExpires && u.subscriptionExpires > Date.now()) {
          const hoursLeft = Math.round((u.subscriptionExpires - Date.now()) / 3600000);
          expiryStr = `<span style="color:#34d399;font-weight:700;">Còn ${hoursLeft} giờ</span>`;
        } else if (u.subscriptionExpires) {
          expiryStr = '<span style="color:var(--accent-rose);">Hết hạn</span>';
        }

        const tr = document.createElement('tr');
        tr.innerHTML = `
          <td class="font-mono" style="font-weight:700;color:var(--accent-cyan);">@${u.username}</td>
          <td><strong>${u.displayName || u.username}</strong></td>
          <td style="font-size:0.78rem;color:var(--text-muted);">${u.createdAt || 'N/A'}</td>
          <td>${roleBadge}</td>
          <td style="font-size:0.8rem;">${expiryStr}</td>
          <td class="font-mono" style="font-size:0.75rem;color:#fde68a;">${u.activeKey || 'Chưa nạp'}</td>
          <td>${statusBadge}</td>
          <td>
            <div style="display:flex;gap:4px;flex-wrap:wrap;">
              <button type="button" class="mini-btn admin-user-grant-btn" data-username="${u.username}" title="Gia hạn thời gian hoặc đổi mật khẩu" style="color:var(--accent-cyan);"><i class="fa-solid fa-wand-magic-sparkles"></i> Sửa / Gia hạn</button>
              <button type="button" class="mini-btn admin-user-toggle-btn" data-username="${u.username}" title="${isBanned ? 'Mở khóa' : 'Khóa'}" style="color:${isBanned ? '#34d399' : 'var(--accent-amber)'};"><i class="fa-solid ${isBanned ? 'fa-unlock' : 'fa-lock'}"></i> ${isBanned ? 'Mở' : 'Khóa'}</button>
              <button type="button" class="mini-btn admin-user-delete-btn" data-username="${u.username}" title="Xóa tài khoản" style="color:var(--accent-rose);"><i class="fa-solid fa-trash"></i></button>
            </div>
          </td>
        `;
        tbody.appendChild(tr);
      ...
      tbody.querySelectorAll('.admin-user-grant-btn').forEach(btn => {
        btn.addEventListener('click', () => {
          sfx.click();
          currentAdminTargetUsername = btn.getAttribute('data-username');
          const targetUserLabel = document.getElementById('adminActionTargetUser');
          if (targetUserLabel) targetUserLabel.textContent = '@' + currentAdminTargetUsername;
          const pwdInput = document.getElementById('adminNewUserPasswordInput');
          if (pwdInput) pwdInput.value = '';
          if (adminUserActionModal) adminUserActionModal.style.display = 'flex';
        });
      });

      tbody.querySelectorAll('.admin-user-toggle-btn').forEach(btn => {
        btn.addEventListener('click', () => {
          sfx.click();
          const uName = btn.getAttribute('data-username');
          const res = UserManager.toggleUserStatus(uName);
          if (res.success) {
            renderAdminUserList();
            showToast(`Đã ${res.status === 'banned' ? 'khóa' : 'mở khóa'} tài khoản @${uName}!`, 'info');
          }
        });
      });

      tbody.querySelectorAll('.admin-user-delete-btn').forEach(btn => {
        btn.addEventListener('click', () => {
          const uName = btn.getAttribute('data-username');
          if (confirm(`Bạn có chắc chắn muốn XÓA VĨNH VIỄN tài khoản @${uName} không?`)) {
            sfx.click();
            UserManager.deleteUser(uName);
            renderAdminUserList();
            updateOsMetrics();
            showToast(`Đã xóa tài khoản @${uName}!`, 'info');
          }
        });
      });
    }

    // Modal Grant Time Handlers
    document.querySelectorAll('.admin-grant-time-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        if (!currentAdminTargetUsername) return;
        sfx.click();
        const hours = parseFloat(btn.getAttribute('data-hours'));
        const res = UserManager.grantUserTime(currentAdminTargetUsername, hours);
        if (res.success) {
          sfx.success();
          renderAdminUserList();
          showToast(`Đã cộng thêm ${hours >= 999999 ? 'Vĩnh viễn' : hours + 'h'} bản quyền cho @${currentAdminTargetUsername}!`, 'success');
          if (adminUserActionModal) adminUserActionModal.style.display = 'none';
        }
      });
    });

    const adminSaveNewPasswordBtn = document.getElementById('adminSaveNewPasswordBtn');
    if (adminSaveNewPasswordBtn) {
      adminSaveNewPasswordBtn.addEventListener('click', () => {
        if (!currentAdminTargetUsername) return;
        const newPass = (document.getElementById('adminNewUserPasswordInput')?.value || '').trim();
        if (!newPass) {
          showToast('Vui lòng nhập mật khẩu mới!', 'error');
          return;
        }
        sfx.click();
        const res = UserManager.resetPassword(currentAdminTargetUsername, newPass);
        if (res.success) {
          sfx.success();
          showToast(`Đã đổi mật khẩu cho @${currentAdminTargetUsername} thành công!`, 'success');
          if (adminUserActionModal) adminUserActionModal.style.display = 'none';
        }
      });
    }

    // 11. Admin Create Seller Action
    ...
    // 12. Handle Custom Hours Input toggle
    ...
    // Quick Restart / Flush Cache
    const quickRestartServerBtn = document.getElementById('quickRestartServerBtn');
    if (quickRestartServerBtn) {
      quickRestartServerBtn.addEventListener('click', () => {
        sfx.click();
        simCpu = 6;
        simRam = 38;
        updateOsMetrics();
        sfx.success();
        showToast('⚡ Đã làm mới toàn bộ bộ nhớ Cache và trạng thái máy chủ thành công!', 'success');
      });
    }

    // 13. Key Filtering & Search
    ...
    // 14. Clear All Keys & Grant All Keys Free Actions
    ...
    const adminGrantAllKeysFreeBtn = document.getElementById('adminGrantAllKeysFreeBtn');
    if (adminGrantAllKeysFreeBtn) {
      adminGrantAllKeysFreeBtn.addEventListener('click', () => {
        let keys = LicenseKeyManager.getKeys();
        if (keys.length === 0) {
          showToast('Chưa có License Key nào trong hệ thống!', 'error');
          return;
        }
        if (confirm(`Bạn có chắc muốn chuyển TOÀN BỘ ${keys.length} License Key thành VĨNH VIỄN (Lifetime)?`)) {
          sfx.click();
          keys.forEach(k => {
            k.duration = 9999;
            k.durationSpec = '9999';
            k.durationLabel = 'Vĩnh viễn (Lifetime)';
            k.expiresAt = 'Vĩnh viễn';
            k.status = 'active';
          });
          LicenseKeyManager.saveKeys(keys);
          renderAdminKeyTable();
          updateOsMetrics();
          sfx.success();
          showToast(`🎉 Đã mở Free Vĩnh Viễn cho toàn bộ ${keys.length} License Key!`, 'success');
        }
      });
    }

    // 15. Admin Save System Settings & Switches
    ...
    const adminAllowRegisterToggle = document.getElementById('adminAllowRegisterToggle');
    const adminOverviewFreeModeToggle = document.getElementById('adminOverviewFreeModeToggle');
    const adminFreeModeToggle = document.getElementById('adminFreeModeToggle');

    const syncAdminSwitches = (settings) => {
      if (adminMaintenanceToggle) adminMaintenanceToggle.checked = !!settings.maintenanceMode;
      if (adminAllowRegisterToggle) adminAllowRegisterToggle.checked = settings.allowRegistrations !== false;
      if (adminOverviewFreeModeToggle) adminOverviewFreeModeToggle.checked = !!settings.freeMode;
      if (adminFreeModeToggle) adminFreeModeToggle.checked = !!settings.freeMode;
      const _autoFreeT = document.getElementById('adminAutoFreeNewUsersToggle');
      if (_autoFreeT) _autoFreeT.checked = !!settings.autoFreeNewUsers;
      const noticeInput = document.getElementById('adminSystemNoticeInput');
      if (noticeInput) noticeInput.value = settings.systemNotice || '';
    };

    [adminMaintenanceToggle, adminAllowRegisterToggle, adminOverviewFreeModeToggle, adminFreeModeToggle].forEach(t => {
      if (t) {
        t.addEventListener('change', () => {
          const cur = AppSettings.get();
          if (t === adminMaintenanceToggle) cur.maintenanceMode = t.checked;
          if (t === adminAllowRegisterToggle) cur.allowRegistrations = t.checked;
          if (t === adminOverviewFreeModeToggle || t === adminFreeModeToggle) {
            cur.freeMode = t.checked;
            if (adminOverviewFreeModeToggle) adminOverviewFreeModeToggle.checked = t.checked;
            if (adminFreeModeToggle) adminFreeModeToggle.checked = t.checked;
          }
          AppSettings.save(cur);
          updateOsMetrics();
          updateLiveSubscriptionTimer();
          showToast('Đã cập nhật công tắc máy chủ!', 'info');
        });
      }
    });
    ...
    if (adminSaveBtn) {
      adminSaveBtn.addEventListener('click', () => {
        ...
        const newSettings = {
          ...AppSettings.get(),
          freeMode: document.getElementById('adminFreeModeToggle')?.checked || false,
          ...
          systemNotice: document.getElementById('adminSystemNoticeInput')?.value.trim() || ''
        };
    ...
    // 16. User Authentication Modal & Profile Logic
    ...
    const guestBannerLoginLink = document.getElementById('guestBannerLoginLink');
    const guestBannerRegisterLink = document.getElementById('guestBannerRegisterLink');
    const guestBannerActionBtn = document.getElementById('guestBannerActionBtn');
    ...
    function renderUserAuthUI() {
      ...
      if (curUser) {
        if (userLabel) userLabel.textContent = `@${curUser.username}`;
        ...
        if (headerQuickLogoutBtn) headerQuickLogoutBtn.style.display = 'inline-flex';
        ...
        if (headerQuickLogoutBtn) headerQuickLogoutBtn.style.display = 'none';
    ...
    const openAuthModal = (tab = 'login') => {
      ...
      if (tab === 'register' && tabUserRegisterBtn) {
        tabUserRegisterBtn.click();
      } else if (tabUserLoginBtn) {
        tabUserLoginBtn.click();
      }
      renderUserAuthUI();
      if (userAuthModal) userAuthModal.style.display = 'flex';
    };

    if (openUserAuthBtn) openUserAuthBtn.addEventListener('click', () => openAuthModal());
    if (guestBannerLoginLink) guestBannerLoginLink.addEventListener('click', (e) => { e.preventDefault(); openAuthModal('login'); });
    if (guestBannerRegisterLink) guestBannerRegisterLink.addEventListener('click', (e) => { e.preventDefault(); openAuthModal('register'); });
    if (guestBannerActionBtn) guestBannerActionBtn.addEventListener('click', () => openAuthModal('login'));
    ...
    if (userSubmitLoginBtn) {
      userSubmitLoginBtn.addEventListener('click', () => {
        ...
        if (res.success) {
          ...
          updateLiveSubscriptionTimer();
          if (userAuthModal) userAuthModal.style.display = 'none';
          showToast(`Đăng nhập thành công! Chào mừng ${res.user.displayName || res.user.username}.`, 'success');
        } else {
          sfx.error();
          if (userAuthErrorMsg) {
    ...
    if (userSubmitRegisterBtn) {
      userSubmitRegisterBtn.addEventListener('click', () => {
        ...
        if (res.success) {
          ...
          updateLiveSubscriptionTimer();
          if (userAuthModal) userAuthModal.style.display = 'none';
          if (res.user.subscriptionExpires >= 9999999999999) {
            showToast(`🎉 Đăng ký thành công! Bạn được kích hoạt Miễn phí vĩnh viễn.`, 'success');
          } else {
            showToast(`🎉 Đăng ký tài khoản @${res.user.username} thành công! Vui lòng nạp Key để sử dụng.`, 'success');
          }
        } else {
          sfx.error();
          if (userAuthErrorMsg) {
    ...
    if (headerQuickLogoutBtn) headerQuickLogoutBtn.addEventListener('click', handleLogout);
    ...
    function updateLiveSubscriptionTimer() {
      ...
      const maintBanner = document.getElementById('globalMaintenanceBanner');

      // Maintenance Banner sync
      if (maintBanner) {
        maintBanner.style.display = settings.maintenanceMode ? 'block' : 'none';
      }
      ...
      if (titleEl) titleEl.textContent = 'THỜI HẠN BẢN QUYỀN';
      const curUser = UserManager.getCurrentUser();

      if (!curUser) {
        if (countEl) countEl.textContent = '🔒 Yêu Cầu Đăng Nhập';
        if (badge) badge.className = 'user-sub-badge is-expired';
        if (profExpEl) profExpEl.textContent = 'Chưa đăng nhập';
        return;
      }
  ...
  function openOsDashboard() {
    ...
    const navUsersBtn = document.getElementById('osNavUsersBtn');
    ...
    if (isRoot) {
      ...
      if (navUsersBtn) navUsersBtn.style.display = 'flex';
    ...
    const settings = AppSettings.get();
    const freeToggle = document.getElementById('adminFreeModeToggle');
    const overviewFreeToggle = document.getElementById('adminOverviewFreeModeToggle');
    const maintToggle = document.getElementById('adminMaintenanceToggle');
    const regToggle = document.getElementById('adminAllowRegisterToggle');
    const noticeInput = document.getElementById('adminSystemNoticeInput');

    if (freeToggle) freeToggle.checked = !!settings.freeMode;
    if (overviewFreeToggle) overviewFreeToggle.checked = !!settings.freeMode;
    if (maintToggle) maintToggle.checked = !!settings.maintenanceMode;
    if (regToggle) regToggle.checked = settings.allowRegistrations !== false;
    if (noticeInput) noticeInput.value = settings.systemNotice || '';
    const autoFreeToggle = document.getElementById('adminAutoFreeNewUsersToggle');
    if (autoFreeToggle) autoFreeToggle.checked = !!settings.autoFreeNewUsers;
    ...
    const overviewBtn = document.querySelector('.os-nav-btn[data-ostab="overview"]');
